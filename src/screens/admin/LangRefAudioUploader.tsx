...
import {
  getLanguageRefAudio,
  uploadLanguageRefAudio,
  deleteLanguageRefAudio,
  REF_SLOTS,
  type RefSlot,
} from '../../data/lessonAudio';

/**
 * Reference audio uploaded per LANGUAGE — applies across every lesson in that
 * language, unlike the per-lesson ref_l2/ref_l1 uploader above. Stored in the
 * public lesson-audio bucket and retrieved by public URL, like step audio.
 */
interface RefState {
  url: string | null;
...
export function LangRefAudioUploader({ language }: { language: string }) {
  ...
  useEffect(() => {
    ...
    void getLanguageRefAudio(language).then((rows) => {
      setStates((prev) => {
        const next = { ...prev };
  ...
  const handleFile = useCallback(
    async (slot: RefSlot, file: File) => {
      ...
      const result = await uploadLanguageRefAudio(language, slot, file, (pct) =>
  ...
  const handleDelete = useCallback(
    async (slot: RefSlot) => {
      ...
      const { error } = await deleteLanguageRefAudio(language, slot);
  ...
  return (
    <section className="mt-8">
      <div className="mb-1 text-[11px] font-bold tracking-[.1em] text-muted">
        REFERENCE AUDIO · PER LANGUAGE
      </div>
      <p className="mb-4 text-[12px] text-muted">
        Applies to every lesson in{' '}
        <span className="font-semibold text-heading">{language || '—'}</span>. Separate from the
        per-lesson reference audio above.
      </p>
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={!language || st.progress !== null}
